...
import {
  Loader2, CheckCircle, ArrowLeft, MapPin, Clock, Wallet, FileText, LocateFixed, Tag,
} from "lucide-react";
...
export default function StageReview({
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-10">
      ...
      {/* Resumen */}
      <Card>
        <h3 className="font-bold text-[13px] tracking-[0.14em] uppercase mb-3" style={{ color: "var(--ink-soft)" }}>Resumen</h3>
        ...
        {promo ? (
          <div className="flex items-center justify-between rounded-md px-3 py-2 mb-2" style={{ background: "rgba(31,170,85,.08)" }}>
        ...
        {selection.giftCardCodigo && (
          <p className="text-[13px] mt-1" style={{ color: "#1a7a40" }}>Tarjeta de regalo {selection.giftCardCodigo} aplicada al confirmar.</p>
        )}
        ...
        <div className="flex justify-between font-bold mt-2" style={{ fontSize: 17, color: "var(--ink)" }}>
      ...
      {/* Contacto */}
      <Card>
        {cardTitle(<BLIcon name="whatsapp" size={16} />, "Datos de contacto")}
      ...
      {orderError && (
        <div className="rounded-md px-4 py-3 text-[14px] font-medium mb-5" style={{ background: "rgba(158,59,70,.08)", color: "var(--berry)" }}>
          {orderError}
        </div>
      )}

      <button onClick={handleConfirm} disabled={submitting}
        className="w-full inline-flex items-center justify-center gap-2 font-bold cursor-pointer border-0 text-white"
