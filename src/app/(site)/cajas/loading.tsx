      ...
      >
        <div className="grid gap-10" style={{ gridTemplateColumns: "1fr 1fr" }}>
          ...
          <div className="flex flex-col gap-5">
            {[1, 2].map((i) => (
              <div key={i} className="rounded-[16px] animate-pulse" style={{ height: 72, background: "var(--cream)" }} />
            ))}
            <div className="grid grid-cols-2 gap-3">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="rounded-[16px] animate-pulse" style={{ height: 90, background: "var(--cream)" }} />
              ))}
            </div>
          </div>
        </div>
